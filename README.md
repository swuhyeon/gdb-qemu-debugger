...
- 현재 선택된 VA에 대해 **페이지 테이블 워크**를 수행하며, 출력 항목은 다음과 같습니다.
  ...
  - `cr3`, `pml4_index`, `pdpt_index`, `pd_index`, `pt_index`, `offset`
...
- 해당 Repository에는 **`qvhd/`와 `scripts/`만** 포함되어 있으며, **`qemu/`, `iso/`, `vm/`은** 아래 **Requirements** 설명에 따라 사용자가 별도로 준비·설정해야 합니다.
...
| Command | 설명                                                                                               |
| ------- | -------------------------------------------------------------------------------------------------- |
| `n`     | `stepi` 실행 (한 instruction씩 실행). 실행 후 **레지스터 + Page Info 갱신** |
| `c`     | `continue` 실행 `is_running = True` 상태가 되며, |
| `p`     | `pause` 실행 중인 게스트를 멈추고 **레지스터 + Page Info 갱신** |
| `r`     | `refresh`. 게스트가 **멈춘 상태에서만** 레지스터 + Page Info를 다시 읽어 옴  |
| `q`     | TUI 종료 & GDB 세션 정리 후 프로그램 종료 |

### 2) Page Info Commands
| Command | 설명                                                                                               |
| ------- | -------------------------------------------------------------------------------------------------- |
| `va rip`    | Page Info 모드를 **RIP-follow 모드**로 전환. 현재 `regs["rip"]` 값을 VA로 사용 |
| `va <addr>` | Page Info 모드를 **수동 모드(`manual`)**로 전환하고. 지정한 VA 기준으로 페이지 정보 출력 |

### 3) Memory Dump Commands
| Command | 설명                                                                                               |
| ------- | -------------------------------------------------------------------------------------------------- |
| `md <va>`        | `<va>` 기준으로 **기본 64바이트** 메모리 덤프 |
| `md <va> <size>` | `<va>` 기준으로 `<size>` 바이트만큼 메모리 덤프 |
